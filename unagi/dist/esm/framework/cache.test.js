...
describe("In-Memory Cache", () => {
  ...
  it("does not cache non-GET requests", async () => {
    ...
    for (const method of ["POST", "PUT", "PATCH", "DELETE"]) {
      ...
      const response = new Response("Hello World");
      await expect(cache.put(request, response)).rejects.toThrow("Cannot cache response to non-GET request");
    }
  });
  it("does not match non-GET requests", async () => {
    ...
    for (const method of ["POST", "PUT", "PATCH", "DELETE"]) {
      ...
      expect(await cache.match(request2)).toBeUndefined();
    }
  });
  it("does not cache responses to a range request (status 206)", async () => {
    ...
    await expect(cache.put(request, response)).rejects.toThrow("Cannot cache response to a range request");
  });
  it("does not cache responses containing vary=* header", async () => {
    ...
    await expect(cache.put(request, response)).rejects.toThrow(`Cannot cache response with 'Vary: *' header.`);
  });
