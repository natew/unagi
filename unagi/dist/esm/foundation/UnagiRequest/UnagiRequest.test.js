...
import { UnagiRequest } from "./UnagiRequest.server.js";
describe("UnagiRequest", () => {
  ...
  it("provides a normalized URL for both RSC and standard requests", () => {
    ...
    const rscRequest = createUnagiRequest(`https://shopify.dev${RSC_PATHNAME}?state=${encodeURIComponent(JSON.stringify({ pathname: "/foo", search: "?bar=baz" }))}`);
